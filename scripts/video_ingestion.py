...
def structure_transcript(client: anthropic.Anthropic, transcript: str,
    ...
    prompt = f"""Ты эксперт-аналитик образовательного контента по ICT трейдингу.

Название стрима/видео: "{video_title}"{chunk_info}

Ниже — транскрипт (или фрагмент транскрипта) обучающего стрима по торговле.
Преобразуй его в структурированный образовательный материал на русском языке.

ТРАНСКРИПТ:
{transcript[:8000]}

Выведи JSON со следующей структурой:
{{
  "title": "Название темы (конкретное, не просто имя файла)",
  ...
  "content": "Полный структурированный текст материала с заголовками ## и маркированными списками. Включи: ключевые концепции, примеры, практические правила."
}}
